...
let panelCache: { html: string; uri: string } | null = null;

/** The panel document and its content-addressed URI, built once per process:
    both inputs are files read once, and /mcp builds a server per request. */
function panel(): { html: string; uri: string } {
  if (panelCache === null) {
