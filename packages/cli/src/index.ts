...
import { resolveTarget, type Target } from "./config.js";
...
async function publish(
  ...
  const t = resolveTarget(root, target, opts.token);
...
async function status(target: string, opts: { token?: string }): Promise<void> {
  const t: Target = resolveTarget(resolve("."), target, opts.token);
...
program
  ...
  .option("--token <token>", "bearer token (overrides the target's env ref)")
