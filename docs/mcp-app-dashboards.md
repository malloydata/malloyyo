...
**Status:** Stages 0–3 are built. `show_dashboard` renders a model's real
dashboards inline in Claude (`src/lib/mcp-app-dashboard.ts`), served from
`/mcp` on the official MCP server SDK (`@modelcontextprotocol/server` v2 +
`@modelcontextprotocol/ext-apps`). Stage 4, the parity pass, is open.
...
`src/app/mcp/route.ts` does bearer auth and builds the per-user surface; the
SDK's `createMcpHandler` owns the protocol — both the 2025 `initialize`
handshake and the 2026-07-28 `server/discover` one. An earlier hand-rolled
JSON-RPC route could not keep up with the latter: 2026-07-28 made `resultType`
and the cache fields (`ttlMs`, `cacheScope`) required on results, and a modern
client that finds them missing rejects the result — it connects and lists zero
tools. Let the SDK track the spec.

**Which clients draw the panel.** In the Claude Desktop Code tab, a panel
renders only for servers that come through Desktop's own MCP config
(`claude_desktop_config.json`, e.g. an `mcp-remote` entry pointing at
`localhost:3000/mcp`). The same server added to Claude Code's `~/.claude.json`
runs `show_dashboard` fine but never draws the panel.
...
**Stage 0 — DONE.** A throwaway prototype (since removed) assembled the panel
document and served it from a test tool. Rendered in Claude Desktop.
...
**Stage 1 — the protocol. DONE.** The `run:` sniff in `runDashboard`, plus the entry
split above. Purely server-side, testable without any panel, and useful to the
existing web frame regardless.

**Stage 2 — the third runtime host. DONE.** `frame-runtime` gains a `callServerTool`
host beside the existing two. `Panel`, `useQuery`, `useGiven` unchanged.

**Stage 3 — resource assembly. DONE** (one runtime resource; the bundle arrives via `dashboard_bundle`, so a changed dashboard does NOT change the URI). `resources/read` returns runtime + bootstrap +
