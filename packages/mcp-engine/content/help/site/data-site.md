...
- **`malloydata/malloyyo-imdb`** — the same, plus a `transform.malloy` that
  ...
  part is `yo_help("site/auto-update")`).
...
Malloy `#@ persist` (see Path B in `yo_help("site/data-to-parquet")`):
...
works. Full detail and copy-paste commands: **`yo_help("site/data-to-parquet")`**.
...
`main` / `docs`). Full steps + gotchas: **`yo_help("site/github-pages")`**.

### 7. (Optional) Keep it fresh automatically

If the data comes from a URL that updates over time and you want the site to
track it, add a weekly GitHub Actions refresh — see
**`yo_help("site/auto-update")`** (worked example: `malloydata/malloyyo-imdb`).
