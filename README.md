...
You develop the model locally with the [Malloy CLI](https://github.com/malloydata/malloy-cli)  Don't worry, Claude (and other AIs) already know the Malloy Language, they same way they know python, so building your semantic model is easy (you can completely vibe code it).  You then publish your model with the [`malloyyo` CLI](packages/cli) (or point Malloyyo at a GitHub repo). Malloyyo compiles it against your Analytical database and serves it as a personal MCP endpoint for claude.ai, Claude Desktop, or any MCP client — running on Vercel or self-hosted in Docker.
