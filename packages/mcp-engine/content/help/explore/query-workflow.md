...
* `query(model_ref: "...", malloy: "run: source -> { ... }", execute: false)` — validate without running; it returns the SQL. Iterate until clean.
...
* `query(model_ref: "...", malloy: "run: source -> { ... }", question: "...")` — run it; get the rows.
