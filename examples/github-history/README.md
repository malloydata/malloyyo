...
Everything GitHub knows about `malloydata/malloy`,
`malloydata/malloy-vscode-extension` and `malloydata/malloyyo`, flattened into
parquet and modelled in Malloy.
...
| --- | ---: | ---: | --- |
| `commits.parquet` | 8,311 | 32 | `repo`, `sha`, `author_email`, `authored_at`, `subject`, `pr_number`, `insertions`, `deletions`, `commit_type` |
| `commit_files.parquet` | 41,811 | 16 | `repo`, `sha`, `path`, `top_dir`, `extension`, `is_test`, `is_docs`, `insertions`, `deletions` |
| `tags.parquet` | 503 | 9 | `repo`, `tag`, `sha`, `tagged_at`, `version` |
| `issues.parquet` | 804 | 21 | `repo`, `issue_number`, `title`, `body`, `state`, `author_login`, `labels`, `comment_count` |
| `pull_requests.parquet` | 3,122 | 27 | `repo`, `pr_number`, `title`, `body`, `state`, `is_merged`, `merged_at`, `head_repo`, `from_fork` |
| `releases.parquet` | 162 | 16 | `repo`, `tag_name`, `published_at`, `body`, `pr_numbers` |
| `identities.parquet` | 106 | 11 | `email`, `login`, `match_source`, `confidence` |

3.3 MB in total. Every `*_at` column is a real UTC timestamp, so the model can
...
`commits.pr_number`. It links 98–99.5% of merged pull requests to the commit
that landed them in all three repos — and it is what puts a real diff size
behind pull request metadata, because GitHub's list-pull-requests API does not
return `additions`/`deletions` at all.

| repo | merged PRs | linked to a commit |
| --- | ---: | ---: |
| `malloydata/malloy` | 1,883 | 99.4% |
| `malloydata/malloy-vscode-extension` | 796 | 99.5% |
| `malloydata/malloyyo` | 125 | 98.4% |
...
| --- | --- | ---: |
| `squash-merge` | the merged PR's author *is* the author of the commit that landed it | 51% |
| `name-match` | a display name that resolves to exactly one login carries over to that person's other addresses | 17% |
| `noreply-address` | `1234567+octocat@users.noreply.github.com` states the login outright | 17% |

85% of commits resolve to a login; the rest are mostly CI bots and one-off
...
python3 extract/git_history.py --out data \
  ...
  --repo ~/dev/malloy-vscode-extension=malloydata/malloy-vscode-extension \
  --repo ~/dev/malloyyo=malloydata/malloyyo

# the API side
GITHUB_TOKEN=ghp_... python3 extract/github_api.py \
  --repo malloydata/malloy \
  --repo malloydata/malloy-vscode-extension \
  --repo malloydata/malloyyo --out raw
...
Also worth knowing: `releases.parquet` holds the most recent 100 releases for
`malloydata/malloy` — `/releases` stopped paginating past the first page there.
`tags.parquet` has all 503 tags from git, so release *cadence* is complete even
